...
* early init
=init.el= 파일과 별개로 =early-init.el= 파일이 존재한다. 이 파일은 =emacs= 가 시작할 때 가장 먼저 로드되며 가장 우선적으로 처리해야할 사항이나 ui가 생성되기 전에 처리해야 할 사항이 있으면 여기에 적용하면 좋다.
ui 요소들 몇개를 비활성화 하고 =package.el= 이 로드되는 것을 비활성화한다.
...
variable의 설명에 =customize= 가 가능하다고 써있으면 =use-package= 의 =:custom= 을 사용하여 설정할 수 있다. =setopt= 를 사용한 것과 같은 효과를 갖는다.
옵션을 켜거나 끌 경우 =+1= =-1= 을 사용하여 조절한다. 단 끄기 위해 =nil= 로 설정해야하는 경우에는 그렇게 지정한다.
...
create definer를 사용해서 리더키를 만들려고 해봤지만 키맵에 override를 적용할 수 없어서 글로벌 리더키로 적용이 안된다.
일일이 넣기 귀찮으니 그냥 원래대로 하기로 했다.
...
**** leader-keys
따로 모아서 정리하고 싶긴 한데 링크를 거는 방법을 까먹어서 일단 보류.
...
누른 키의 정보를 알려주는 패키지. 모드라인에 추가해서 표시할 수 있다.
일단 활성화에 실패해 보류중
#+begin_src emacs-lisp :lexical t
  (use-package keycast
      ;; :disabled t
...
*** kdl
niri 설정 파일은 kdl 이라는 확장자를 사용한다. 이 확장자를 위한 메인 모드이다.
#+begin_src emacs-lisp :lexical t
...
=vc-jj= 와 =jj-mode= 두 가지 패키지가 있다.
=jj-mode= 가 좀 더 사용하기 편한 감이 있어서 =vc-jj= 대신 =jj-mode= 로 결정.
#+begin_src emacs-lisp :lexical t
  (use-package vc-jj
      :demand t
      :disabled t
...
*** magit
+=git= 을 사용하기 위한 패키지. 일단은 =vc= 를 사용해보기 위해 비활성화 했다.+
=jj-mode= 를 사용하기 위해서 의존성으로 들어간다.
...
기본을 =flex= 로 설정해서 찾으려는 단어가 짧을 때는 앞에 ~=~ 을 붙여서 글자 그대로 검색하는 게 가능하다.
...
*** cape
자동완성을 제공해주는 패키지?
=dobin/elisp-mode-init= 은 =use-package= 에서 =:custom= 부분을 수정할 때 함수가 아닌 =custom= 에 맞는 자동완성을 제공한다.
...
- 출처 : https://codepractice.tistory.com/167
  ...
  emacs 단독으로 실행했을 때에는 폰트가 적용이 안되는 이슈가 있어서 훅을 추가했다.
...
** use "+" register
기본적으로는 =*= 레지스터를 사용하고 원할 때만 =+= 레지스터를 사용하여 클립보드를 보존하는 기능
#+begin_src emacs-lisp :lexical t
...
#+end_src
** custom macros
evil mode에서 input을 시뮬레이션 하기 위해 사용할 함수를 만드는 매크로.
레지스터를 시스템 클립보드랑 분리하고 리더키를 통해 손쉽게 접근하기 위해 단축키를 만들었다. 다른 키입력을 시뮬레이션(?) 할 때 사용할 수 있다.

defmacro는 defun과 달리 먼저 평가되어야 하기에 위쪽에서 noweb ref로 삽입했다.
