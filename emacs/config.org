...
#+begin_src emacs-lisp :lexical t
  (use-package emacs
      ...
      (defun dobin/maximize-frame (frame)
          "Maximazes the active frame in Windows"
          (interactive)
          (modify-frame-parameters frame '((window-state . maximized))))

      :config
      ;; ref: https://emacs.stackexchange.com/a/3008/49425
      (add-to-list 'default-frame-alist '(window-state . maximized))
      ;; (add-hook 'after-make-frame-functions 'dobin/maximize-frame)

      :hook
      (before-save . delete-trailing-whitespace) ; 저장 시 남은 공백 제거
      (window-setup . dobin/maximize-frame)
      ;; (after-make-frame-functions . dobin/maximize-frame)
      )
#+end_src
